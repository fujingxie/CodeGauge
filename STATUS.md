...
- Android UI-R3: `./gradlew :android:app:testDebugUnitTest :android:app:assembleDebug` 通过。
- Android UI-R3: adb 真机复测通过，已安装 debug APK、截图检查 Activity / Settings；`logcat` 未发现 `AndroidRuntime` / `FATAL EXCEPTION` / `CodeGaugeActivity` 异常日志。
- Android UI-R4: `./gradlew :android:app:testDebugUnitTest :android:app:assembleDebug` 通过。
...
- T16 安装脚本目前面向 macOS LaunchAgent；Windows/Linux 后台安装流程未做。
- T16 `codegaugectl pair-code` 依赖 Companion 日志中的启动配对码；如果日志轮转或尚未启动，需要先查看 `codegaugectl logs` 或重启服务。
...
- T16 macOS 安装层采用 `~/.codegauge` 作为自包含安装目录，LaunchAgent 只执行 `codegauge-runner`，由 runner source `codegauge.env` 后 exec Companion，避免在 plist 内维护大量环境变量和 XML 转义风险。
