...
7. **首页额度偏好**：Dashboard 默认优先展示 5H 窗口，用户可在 Settings 里切换首页主额度为 5H 或周额度，偏好保存到 Companion settings
...
- Release 签名读取本机 `keystore.properties`，长期密钥放在 `release/codegauge-release.jks` 或用户指定路径；`keystore.properties` 和 `*.jks` 不提交仓库
...
1. **产品化发布**：Android debug 版本、Android release 签名流程、Companion macOS LaunchAgent 安装流程已可用；后续重点是真实长期 keystore 下的 release 真机回归、安装体验打磨和更严格 LAN/TLS 策略
